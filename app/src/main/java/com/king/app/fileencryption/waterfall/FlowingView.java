 ...
 *
 */
public class FlowingView extends View implements View.OnLongClickListener {
	...
	public void loadImage() {
		imageBmp = imageProvider.loadImage(imageFilePath);
		if (imageBmp != null) {
			createDrawRect();
		}
	}

	private void createDrawRect() {
		if (imageBmp != null) {
			int bmpWidth = imageBmp.getWidth();
			int bmpHeight = imageBmp.getHeight();
			height = (int) (bmpHeight * width / bmpWidth);
			if (height > width*3) {//avoid too high bitmap
				height = width * 3;
			}
